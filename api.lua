...
outdoor.getFormSpec = function(numSelected)
	local formspec = "size[4.3,3.7]"
		...
		.."button_exit[0,3.4;4.2,0.4;btnSelect;SET ITEM]"
	--print(formspec)
	return formspec
end
	...
	minetest.register_node("outdoor:"..id, {
		...
		on_rightclick = function(pos, node, clicker)
			...
			if outdoor.canUse(pos, clicker) then
				...
				minetest.chat_send_player(playername,"Can not modify. Owned by '"..ownername.."'!")
			end
		end,
		--]]
		--[[
			...
			if fields.btnSelect ~=nil then
				...
				if outdoor.canUse(pos, sender) then
					if type(outdoor.selected)=="number" and outdoor.selected>=1 then
						...
						minetest.log('action',sender:get_player_name().." selected outdoor '"..description.."' em "..minetest.pos_to_string(pos))
					...
					local meta = minetest.get_meta(pos)
					minetest.chat_send_player(sender:get_player_name(),"Can not modify. Owner is '"..meta:get_string("owner").."'!")
				end
		...
		can_dig = function(pos, digger)
			...
			if canDig == false then
				local meta = minetest.get_meta(pos)
				minetest.chat_send_player(digger:get_player_name(),"Can not modify. Owner is '"..meta:get_string("owner").."'!")
			end
